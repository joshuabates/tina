# Mechanical Review Workbench Phase 5.5 Remediation Plan

> **For Claude:** Use tina:executing-plans to implement this plan.

**Plan Baseline:** 9454c40cfb3081df3d84ed4ef679384d2d34e29d

**Goal:** Address gaps from Phase 5 review: Phase 5 was never executed. This remediation implements the full Checks Tab — a real-time subscription-driven display of `reviewChecks` data showing status badges (running/passed/failed), durations, failure output, and live fill-in as the review agent works through its checklist.

**Architecture:** Targeted fixes to existing implementation. No new architecture.

**Phase context:** Phase 5 was planned but never executed. The original plan at `docs/plans/2026-02-13-mechanical-review-workbench-design-phase-5.md` is correct and well-structured. Phase 4 built `ReviewDetailPage` with tab shell, `ConversationTab`, `CommitListPanel`, review query defs, and test infrastructure. Phase 1 built the Convex `reviewChecks` table with `startCheck`, `completeCheck`, and `listChecksByReview`. This remediation connects them with frontend code: Effect schema, query definition, test builder, check status styles, and `ChecksTab` component.

**Issues to address:**
1. Missing `ReviewCheck` Effect schema — no frontend type for reviewChecks data
2. Missing `ReviewCheckListQuery` query definition — no way to subscribe to checks
3. Missing check status styles (running/passed/failed) — `toStatusBadgeStatus` falls back for these values
4. Missing `buildReviewCheck` test builder — can't write tests for checks UI
5. Missing `ChecksTab` component — Checks tab shows placeholder text
6. `ReviewDetailPage` still renders placeholder for Checks tab instead of `ChecksTab` component
...
|------|-------------------|
| Task 1: ReviewCheck schema + query def + status styles | 3 |
...
1. Create `tina-web/src/schemas/reviewCheck.ts`:
...
2. In `tina-web/src/schemas/index.ts`, add after the `ReviewGate` export line:

```typescript
export { ReviewCheck } from "./reviewCheck"
```

3. In `tina-web/src/services/data/queryDefs.ts`, add `ReviewCheck` to the import from `@/schemas`:

Change the import to include `ReviewCheck`:
```typescript
import {
  // ... existing imports ...
  ReviewCheck,
...
Then add after `ReviewGateListQuery`:
...
4. In `tina-web/src/components/ui/status-styles.ts`, add to `STATUS_VALUES` array after `"superseded"`:
...
And add to `statusStyleMap` after the `superseded` entry (before `// Ticket statuses`):
...
1. In `tina-web/src/test/builders/domain/entities.ts`, add `ReviewCheck` to the type import from `@/schemas` (line 1-12). The import block should include `ReviewCheck` alongside existing types.

2. Add builder function after `buildReviewGate` (after line 210):
...
1. In `tina-web/src/components/ReviewDetailPage.tsx`, add import after line 14 (`import { ConversationTab } from "./ConversationTab"`):

```typescript
import { ChecksTab } from "./ChecksTab"
```

2. Replace the checks placeholder (lines 153-157). Find:
...
3. In `tina-web/src/components/__tests__/ReviewDetailPage.test.tsx`, add ChecksTab mock after the ConversationTab mock (after line 30):
...
4. Update the "shows placeholder when switching to Checks tab" test (lines 141-155). Find:
...
5. Run all review-related tests:
...
6. Run full tina-web type check:
