...
describe("fromDispatchErrorCode", () => {
  test("maps PayloadMissingField to dispatch_payload_invalid", () => {
    expect(fromDispatchErrorCode("PayloadMissingField")).toBe(
      REASON_CODES.DISPATCH_PAYLOAD_INVALID,
    );
  });

  test("maps PayloadInvalid to dispatch_payload_invalid", () => {
    expect(fromDispatchErrorCode("PayloadInvalid")).toBe(
      REASON_CODES.DISPATCH_PAYLOAD_INVALID,
    );
  });

  test("maps UnknownActionType to dispatch_unknown_type", () => {
    expect(fromDispatchErrorCode("UnknownActionType")).toBe(
      REASON_CODES.DISPATCH_UNKNOWN_TYPE,
    );
  });

  test("maps CliExitNonZero to dispatch_cli_exit_nonzero", () => {
    expect(fromDispatchErrorCode("CliExitNonZero")).toBe(
      REASON_CODES.DISPATCH_CLI_EXIT_NONZERO,
    );
  });

  test("maps CliSpawnFailed to dispatch_cli_spawn_failed", () => {
  ...
  test("falls back to dispatch_payload_invalid for unknown codes", () => {
    expect(fromDispatchErrorCode("SomethingElse")).toBe(
      REASON_CODES.DISPATCH_PAYLOAD_INVALID,
    );
  });
});
